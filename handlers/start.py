...
from services.google_auth import get_credentials, get_token_path, get_url
...
async def start_command(message: types.Message):
    ...
    user_id = message.from_user.id  # Убираем await
    get_url(user_id)
    creds, auth_url = await get_credentials(user_id)  # 🔥 Получаем авторизационные данные
    token_path = get_token_path(user_id)  # Получаем путь к токену


    
    if auth_url:
        # Если требуется авторизация, отправляем ссылку
        logging.info(f"🔗 Отправляем ссылку: {auth_url}")
